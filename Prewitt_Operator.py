    ...
    def prewitt(self,image):
        ...
        for r in range(2,row-2):
            for c in range(2,col-2):
                image_y[r][c]=image[r-1][c-1]*-1 + image[r][c-1]*-1 + image[r+1][c-1]*-1 + \
                             image[r-1][c+1]*1+ image[r][c+1]*1 + image[r+1][c+1]*1
                image_x[r][c] = image[r - 1][c - 1] * -1 + image[r + 1][c - 1]*1  + image[r - 1][c] * -1 +\
                               image[r + 1][c] * 1 +  image[r - 1][c + 1] * -1  +  image[r + 1][c + 1]*1
